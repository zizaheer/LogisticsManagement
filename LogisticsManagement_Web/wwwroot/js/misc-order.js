    ...
    if (serviceId !== '0' && serviceId !== 'undefined') {
        ...
        if (addServiceInfo != null && addServiceInfo != '' && addServiceInfo !== 'null') {
            ...
            if (serviceInfo.IsPriceApplicable == true) {
                currentRow.find('td:eq(1) .txtUnitPrice').val(serviceInfo.UnitPrice);
                currentRow.find('td:eq(2) .txtQuantity').val("1");
                currentRow.find('td:eq(1) .txtUnitPrice').prop("disabled", false);
                currentRow.find('td:eq(2) .txtQuantity').prop("disabled", false);
                currentRow.find('td:eq(3) .txtServiceFee').val(serviceInfo.UnitPrice * 1.0);
                currentRow.find('td:eq(0) .txtAdditionalServiceName').data("isunitpriceapplicable", "1");

            } else {
                currentRow.find('td:eq(1) .txtUnitPrice').val("");
                currentRow.find('td:eq(2) .txtQuantity').val("");
                currentRow.find('td:eq(1) .txtUnitPrice').prop("disabled", true);
                currentRow.find('td:eq(2) .txtQuantity').prop("disabled", true);
                currentRow.find('td:eq(3) .txtServiceFee').val("");
                currentRow.find('td:eq(0) .txtAdditionalServiceName').data("isunitpriceapplicable", "0");
            }

            currentRow.find('td:eq(0) .hfServiceId').val(serviceId);
            currentRow.find('td:eq(4) .chkIsGstApplicableForService').prop('checked', serviceInfo.IsTaxApplicable);
            currentRow.find('td:eq(6) .btnAddAdditionalService').attr('data-serviceid', serviceId);
            currentRow.find('td:eq(6) .btnDeleteAdditionalService').attr('data-serviceid', serviceId);
        }
    }
...
$('#service-list').on('click', '.chkIsUpdate', function () {
    ...
    if (isChecked == true) {
        selectedRow.find('td:eq(6) .btnEditService').prop("disabled", false);
        selectedRow.find('td:eq(0) .txtAdditionalServiceName').prop("disabled", false);
        selectedRow.find('td:eq(3) .txtServiceFee').prop("disabled", false);
        selectedRow.find('td:eq(4) .chkIsGstApplicableForService').prop("disabled", false);

        var isunitpriceapplicable = selectedRow.find('.txtAdditionalServiceName').data("isunitpriceapplicable");
        if (isunitpriceapplicable == "1") {
            selectedRow.find('td:eq(1) .txtUnitPrice').prop("disabled", false);
            selectedRow.find('td:eq(2) .txtQuantity').prop("disabled", false);
        }
        else {
            selectedRow.find('td:eq(1) .txtUnitPrice').prop("disabled", true);
            selectedRow.find('td:eq(2) .txtQuantity').prop("disabled", true);
        }
    }
    else {
        selectedRow.find('td:eq(6) .btnEditService').prop("disabled", true);
        selectedRow.find('td:eq(0) .txtAdditionalServiceName').prop("disabled", true);
        selectedRow.find('td:eq(1) .txtUnitPrice').prop("disabled", true);
        selectedRow.find('td:eq(2) .txtQuantity').prop("disabled", true);
        selectedRow.find('td:eq(3) .txtServiceFee').prop("disabled", true);
        selectedRow.find('td:eq(4) .chkIsGstApplicableForService').prop("disabled", true);
    }
...
function addAdditionalServiceRowData(currentRow) {
    ...
    var serviceId = currentRow.find('td:eq(0) .hfServiceId').val();
    var serviceName = currentRow.find('td:eq(0) .txtAdditionalServiceName').val();
    var serialNo = parseInt(currentRow.find('td:eq(1) .hfSerialNo').val());

    var serviceFee = currentRow.find('td:eq(3) .txtServiceFee').val();
    ...
    var unitQty = currentRow.find('td:eq(2) .txtQuantity').val();
    var unitPrice = currentRow.find('td:eq(1) .txtUnitPrice').val();
    ...
    currentRow.find('td:eq(1) .txtUnitPrice').prop("disabled", true);
    currentRow.find('td:eq(2) .txtQuantity').prop("disabled", true);
    currentRow.find('td:eq(3) .txtServiceFee').prop("disabled", true);
    currentRow.find('td:eq(4) .chkIsGstApplicableForService').prop("disabled", true);
    currentRow.find('td:eq(5) .chkIsUpdate').prop("disabled", false);
    currentRow.find('td:eq(5) .chkIsUpdate').prop("checked", false);

    currentRow.find('td:eq(6) .btnAddAdditionalService').prop("disabled", true);
    currentRow.find('td:eq(6) .btnEditService').prop("disabled", true);
    currentRow.find('td:eq(6) .btnDeleteAdditionalService').removeAttr('disabled');
}
    ...
    if (orderAdditionalServiceData !== null) {
        for (var i = 0; i < orderAdditionalServiceData.length; i++) {
            ...
            if (additionalServiceInfo !== null && additionalServiceInfo !== undefined && additionalServiceInfo !== '') {
                ...
                currentRow.find('td:eq(0) .hfServiceId').val(serviceData.additionalServiceId);
                currentRow.find('td:eq(0) .txtAdditionalServiceName').val(additionalServiceInfo.ServiceName);
                currentRow.find('td:eq(1) .hfSerialNo').val(serviceData.serialNo);

                if (serviceData.unitPrice > 0) {
                    currentRow.find('td:eq(1) .txtUnitPrice').val(serviceData.unitPrice);
                } else {
                    currentRow.find('td:eq(1) .txtUnitPrice').val("");
                }
                if (serviceData.quantity > 0) {
                    currentRow.find('td:eq(2) .txtQuantity').val(serviceData.quantity);
                } else {
                    currentRow.find('td:eq(2) .txtQuantity').val("");
                }
                ...
                currentRow.find('td:eq(3) .txtServiceFee').val(serviceData.additionalServiceFee);
                ...
                currentRow.find('td:eq(6) .btnAddAdditionalService').prop('disabled', true);
                currentRow.find('td:eq(6) .btnEditService').prop('disabled', true);
                currentRow.find('td:eq(6) .btnDeleteAdditionalService').attr('data-serviceid', serviceData.additionalServiceId);
            }
        }
    }
}
...
function GenerateNewAdditionalServiceRow() {
    ...
    appendString += '<td style="width:215px">';
    appendString += '<input type="hidden" class="hfServiceId" name="hfServiceId">';
    appendString += '<input class="form-control form-control-sm additionalServiceControl txtAdditionalServiceName" id="txtAdditionalServiceName" data-isunitpriceapplicable="0" style="width:200px; margin-left:3px" placeholder="Service name" list="additionalServices" type="search" />';
    appendString += '<datalist id="additionalServices" class="additionalServices">';
    appendString += '</datalist>';
    appendString += '</td>';
    appendString += '<td style="width:70px;">';
    appendString += '<input type="hidden" class="hfSerialNo" name="hfSerialNo" value="' + additionalServiceSerial + '">';
    appendString += '<input type="number" class="form-control form-control-sm additionalServiceControl txtUnitPrice" style="width:65px;" step=".01" placeholder="Prc." title="Unit price" disabled />';
    appendString += '</td>';
    appendString += '<td style="width:54px;">';
    appendString += '<input type="number" class="form-control form-control-sm additionalServiceControl txtQuantity" style="width:45px;" step=".01" placeholder="Qty." title="Quantity" disabled />';
    appendString += '</td>';
    appendString += '<td style="width:81px;">';
    appendString += '<input type="number" class="form-control form-control-sm additionalServiceControl txtServiceFee " id="txtServiceFee"  style="width:80px;" step=".01" name="txtServiceFee" placeholder="Fee" title="Applicable service fee amount" />';
    appendString += '</td>';
    appendString += '<td style="width:85px; text-align:center">';
    ...
    appendString += '</td>';
    appendString += '<td style="width:96px; text-align:center">';
    appendString += '<input type="checkbox" class="chkIsUpdate" name="chkIsUpdate" disabled />';
    appendString += '</td>';
    appendString += '<td style="width:90px;text-align:right;">';
    appendString += '<button class="btn btn-sm btn-primary additionalServiceControl btnAddAdditionalService" type="button" id="btnAddAdditionalService" name="btnAddAdditionalService" title="Click to add to order"><i class="fa fa-plus-circle"></i> </button>';
    appendString += '&nbsp;';
    appendString += '<button class="btn btn-sm btn-success additionalServiceControl btnEditService" type="button" title="Update & save" disabled><i class="fa fa-save"></i> </button>';
    appendString += '&nbsp;';
    appendString += '<button class="btn btn-sm btn-danger additionalServiceControl btnDeleteAdditionalService" type="button" name="btnDeleteAdditionalService" title="Delete from order"><i class="fa fa-trash"></i> </button>';
    appendString += '</td>';
    appendString += '</tr>';
    appendString += '</tbody>';
